  ...
  return (
    <div className={`min-h-screen bg-[#F6F6F6] selection:bg-[#26B7FF]/30 ${isRtl ? 'text-right' : 'text-left'}`} dir={isRtl ? 'rtl' : 'ltr'}>
      ...
      <main className="max-w-[1140px] mx-auto px-6 py-16 space-y-24">
        ...
        {/* Hero Section - 完美对齐参考样式 Screenshot 2.19.24 PM */}
        <section className="text-center space-y-8">
          ...
          <div className="max-w-3xl mx-auto bg-white p-10 rounded-[24px] shadow-card border-s-8 border-[#26B7FF] relative overflow-hidden text-start">
             <div className="flex items-start gap-5">
                <Sparkles className="text-[#FDE700] shrink-0 mt-1" size={32} />
                <p className="text-xl text-[#666666] leading-relaxed font-medium">
        ...
        {/* Transformation Section - 三卡片进度条布局 */}
        <section className="space-y-12">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 bg-[#26B7FF]/10 rounded-2xl flex items-center justify-center text-[#26B7FF]">
              <TrendingUp size={24} />
            </div>
            <h2 className="text-2xl font-bold">{t("The Transformation: Before & After", "التحول: قبل وبعد")}</h2>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {reportData.transformation.map((item, i) => (
              <div key={i} className="bg-white rounded-[24px] p-8 shadow-card border border-white hover:border-[#26B7FF]/20 transition-all group">
                ...
                <div className="h-3 bg-[#F6F6F6] rounded-full mb-4 overflow-hidden">
                  <motion.div 
                    ...
                    className="h-full bg-[#26B7FF] rounded-full"
                  />
                </div>
        ...
        {/* Final Encouragement - 黑色品牌卡片布局 */}
        <section className="text-center p-20 bg-[#26B7FF] rounded-[40px] text-white space-y-8 shadow-2xl relative overflow-hidden">
          ...
          <h2 className="text-2xl font-bold max-w-2xl mx-auto leading-relaxed italic">
            "{t(reportData.encouragement.en, reportData.encouragement.ar)}"
          </h2>
          <div className="inline-block px-10 py-3 bg-[#FDE700] text-[#333333] rounded-full font-bold shadow-xl hover:scale-105 transition-transform cursor-default">
            EXCELLENT PROGRESS
          </div>
        </section>
