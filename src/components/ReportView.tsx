...
export default function ReportView({ student }: { student: any }) {
  ...
  const reportData = useMemo(() => {
    ...
    const transformation = transRaw.split(/Category:/i).filter(Boolean).map(block => {
      const lines = block.trim().split('\n');
      const cat = lines[0].trim();
      const beforePart = lines.find(l => l.startsWith('Before:'))?.replace('Before:', '').split('|') || [];
      const afterPart = lines.find(l => l.startsWith('After:'))?.replace('After:', '').split('|') || [];
      return {
        category: cat,
    ...
    const gaps = gapRaw.split('\n').filter(Boolean).map(line => {
      const [id, title, desc] = line.split('|').map(s => s.trim());
    ...
    const phases = roadmapRaw.split('\n').filter(Boolean).map(line => {
      const [phase, name, module, why] = line.split('|').map(s => s.trim());
      return { phase, name, module, why };
    });
      ...
      <main className="max-w-[1140px] mx-auto px-6 py-16 space-y-24">
            ...
            {reportData.transformation.map((item, i) => (
              <div key={i} className="bg-white p-8 rounded-[24px] shadow-sm grid grid-cols-1 md:grid-cols-10 items-center gap-6 border border-white hover:border-[#26B7FF]/10 transition-all">
                ...
                <div className="md:col-span-4 bg-[#F6F6F6] p-6 rounded-2xl">
                  ...
                  <p className="text-[14px] text-[#666666] mt-1">{item.beforeSub}</p>
                </div>
                <div className="md:col-span-4 bg-[#26B7FF]/5 p-6 rounded-2xl border border-[#26B7FF]/10">
                  ...
                  <p className="text-[14px] text-[#26B7FF] font-medium mt-1">{item.afterSub}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <section className="bg-[#282828] text-white rounded-[40px] p-8 md:p-20 relative overflow-hidden shadow-2xl">
          ...
          <div className="relative z-10 space-y-16">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 text-start">
              ...
              <button onClick={() => setShowFullGap(!showFullGap)} className="bg-white/10 hover:bg-white/20 text-white px-8 py-3 rounded-full text-sm font-bold transition-all border border-white/10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-12 text-start">
              {reportData.gaps.map((gap, idx) => (
                <div key={idx} className="space-y-6 group">
                  ...
                  {showFullGap && <p className="text-white/50 text-sm leading-relaxed animate-in fade-in slide-in-from-top-2">{gap.desc}</p>}
        ...
        <section className="space-y-12">
          <div className="flex items-center gap-4 px-2">
            ...
            <h2 className="text-[28px] font-bold text-[#333333]">{t("Custom Learning Roadmap", "خارطة الطريق")}</h2>
          </div>
            ...
            {reportData.phases.map((p, idx) => (
              <div key={idx} className="bg-white p-8 md:p-12 rounded-[32px] shadow-sm border-t-[10px] border-[#FDE700] space-y-8">
                ...
                <div className="space-y-4">
                  <div className="bg-[#F6F6F6] p-4 rounded-xl border-s-4 border-[#26B7FF]">
                    <span className="text-[12px] font-bold text-[#26B7FF] uppercase">Selected Module</span>
                    <p className="text-[#333333] font-bold mt-1">{p.module}</p>
                  </div>
                  <div className="flex gap-4">
                    <CheckCircle2 className="text-[#26B7FF] shrink-0" size={20} />
                    <p className="text-[#666666] text-[15px] italic leading-relaxed">{p.why}</p>
                  </div>
                </div>
              </div>
            ))}
        ...
        <section className="text-center p-12 md:p-24 bg-[#26B7FF] rounded-[48px] text-white space-y-10 shadow-2xl relative overflow-hidden group">
          ...
          <div className="inline-block px-12 py-4 bg-[#FDE700] text-[#333333] rounded-full font-black text-lg shadow-2xl shadow-black/20 transform hover:scale-105 transition-all cursor-default uppercase tracking-widest">
