...
import { ArrowLeft, Award, TrendingUp, User, AlertTriangle, Map, GraduationCap, Calendar } from 'lucide-react';

export default function ReportDetailPage() {
  const [lang, setLang] = useState<'en' | 'ar'>('en');
  const isRtl = lang === 'ar';

  // 这里的学生数据将来可以从 API 获取，目前先用 Ziad 的真实内容
  const data = {
    name: "Ziad",
    studentId: "62283399",
    date: "2026-03-05",
    ...
    <div className={`min-h-screen bg-[#F6F6F6] text-[#333333] pb-20 font-poppins`} dir={isRtl ? 'rtl' : 'ltr'}>
      ...
      <header className="bg-white border-b border-gray-100 px-6 py-4 sticky top-0 z-50">
        <div className="max-w-[1140px] mx-auto flex justify-between items-center">
          ...
          <button 
            ...
            className="px-5 py-2 border-2 border-[#26B7FF] text-[#26B7FF] rounded-[50px] font-semibold text-sm hover:bg-[#26B7FF] hover:text-white transition-all"
          >
      ...
      <main className="max-w-[1140px] mx-auto px-6 pt-10 space-y-16">
        ...
        <section className="text-center space-y-4 max-w-3xl mx-auto">
          ...
          <h2 className="text-[41px] font-bold text-[#333333] leading-tight">
            {t.title}
          </h2>
          <p className="text-[20px] text-[#666666] leading-relaxed">
            {t.summary}
          </p>
        </section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {[
            ...
            <div key={i} className="bg-white rounded-[20px] p-8 shadow-sm border border-gray-50 hover:shadow-md transition-shadow">
              <div className="flex items-center gap-3 mb-6">
                <div className="p-2 bg-blue-50 rounded-lg">{item.icon}</div>
                <h4 className="text-[20px] font-bold">{item.label}</h4>
              </div>
              <div className="h-1.5 bg-gray-100 rounded-full mb-4">
                <div className="h-full bg-[#26B7FF] rounded-full w-[85%]"></div>
              </div>
        ...
        <section className="bg-white rounded-[20px] p-8 md:p-12 shadow-sm border border-gray-50">
          <div className="flex items-center gap-4 mb-8">
            ...
            <h3 className="text-[32px] font-bold">{t.gapTitle}</h3>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-[14px]">
            <div className="p-6 bg-[#F6F6F6] rounded-[20px] border-s-8 border-[#FDE700]">
        ...
        <section className="space-y-8">
          <div className="flex items-center gap-4">
            ...
            <h3 className="text-[32px] font-bold">{t.roadmapTitle}</h3>
          </div>
          <div className="space-y-6">
            ...
            <div className="bg-white p-8 rounded-[20px] border-2 border-gray-100 opacity-60">
